...
namespace Mvc.Utility.Core.Managers.Authorization
{
    ...
    public class AuthorizationPolicy
    {
            ...
            IEnumerable<string> authenticationSchemes)
        {
            ...
            if (authenticationSchemes == null) throw new ArgumentNullException(nameof(authenticationSchemes));

            var requirementsList = new List<IAuthorizationRequirement>(requirements);
            if (requirementsList.Count == 0)
                throw new InvalidOperationException(ShareResources.Exception_AuthorizationPolicyEmpty);
        ...
        public static AuthorizationPolicy Combine(IEnumerable<AuthorizationPolicy> policies)
        {
            ...
            foreach (var policy in policies) builder.Combine(policy);
            ...
            foreach (var authorizeAttribute in authorizeData)
            {
                ...
                if (!string.IsNullOrWhiteSpace(authorizeAttribute.Policy))
                {
                    ...
                    if (policy == null)
                        throw new InvalidOperationException(
                            ResourceHelper.FormatException_AuthorizationPolicyNotFound(authorizeAttribute.Policy));
                ...
                if (!string.IsNullOrWhiteSpace(authorizeAttribute.ActiveAuthenticationSchemes))
                {
                    policyBuilder.AddAuthenticationSchemes(
                        SplitAndTrim(authorizeAttribute.ActiveAuthenticationSchemes));
                    useDefaultPolicy = false;
                }
        ...
        private static string[] SplitAndTrim(string commaSeparated)
        {
            ...
            for (var i = 0; i < split.Length; i++) split[i] = split[i].Trim();
