...
namespace Mvc.Utility.Core.Managers.UrlManager
{
    ...
    public class Url
    {
        ...
        /// </summary>
        public QueryProvider QueryParams { get; private set; }
        ...
        public static QueryProvider ParseQueryParams(string query)
        {
            var result = new QueryProvider();
        ...
        public static string GetRoot(string url)
        {
            ...
            return new Uri(url).GetComponents(UriComponents.SchemeAndServer | UriComponents.UserInfo,
                UriFormat.Unescaped);
        }
        ...
        {
            foreach (var segment in segments) AppendPathSegment(segment);
        ...
        {
            foreach (var s in segments) AppendPathSegment(s);
        ...
        public Url SetQueryParam(string name, string value, bool isEncoded = false,
            NullValueHandling nullValueHandling = NullValueHandling.Remove)
        {
        ...
        public Url SetQueryParams(object values, NullValueHandling nullValueHandling = NullValueHandling.Remove)
        {
            ...
            foreach (var kv in values.ToKeyValuePairs()) QueryParams.Merge(kv.Key, kv.Value, false, nullValueHandling);
        ...
        {
            foreach (var name in names.Where(n => n != null)) SetQueryParam(name);
        ...
        {
            foreach (var name in names) QueryParams.Remove(name);
        ...
        {
            var sb = new StringBuilder(encodeSpaceAsPlus ? Path.Replace("%20", "+") : Path);
            if (Query.Length > 0) sb.Append("?").Append(QueryParams.ToString(encodeSpaceAsPlus));
        ...
        {
            return obj is Url url && ToString().Equals(url.ToString());
        }
