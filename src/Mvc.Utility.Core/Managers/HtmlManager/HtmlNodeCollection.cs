...
namespace Mvc.Utility.Core.Managers.HtmlManager
{
    ...
    public class HtmlNodeCollection : IList<HtmlNode>
    {
        ...
        public void Add(HtmlNode node, bool setParent)
        {
            ...
            if (setParent) node.ParentNode = _parentnode;
        }
        ...
        public void Insert(int index, HtmlNode node)
        {
            ...
            if (index > 0) prev = _items[index - 1];

            if (index < _items.Count) next = _items[index];

            _items.Insert(index, node);

            if (prev != null)
            {
                if (node == prev) throw new InvalidProgramException("Unexpected error.");

                prev._nextnode = node;
            }

            if (next != null) next._prevnode = node;

            node._prevnode = prev;
            if (next == node) throw new InvalidProgramException("Unexpected error.");
        ...
        /// <summary>
        ///     Remove <see cref="Mvc.Utility.Core.Managers.HtmlManager.HtmlNode" /> at index
        /// </summary>
        /// <param name="index"></param>
        public void RemoveAt(int index)
        {
            ...
            if (index < _items.Count - 1) next = _items[index + 1];

            _items.RemoveAt(index);

            if (prev != null)
            {
                if (next == prev) throw new InvalidProgramException("Unexpected error.");

                prev._nextnode = next;
            }

            if (next != null) next._prevnode = prev;
        ...
        {
            HtmlNode last = null;
            if (_items.Count > 0) last = _items[_items.Count - 1];
            ...
            if (last == node) throw new InvalidProgramException("Unexpected error.");
        ...
        {
            HtmlNode first = null;
            if (_items.Count > 0) first = _items[0];

            _items.Insert(0, node);

            if (node == first) throw new InvalidProgramException("Unexpected error.");

            node._nextnode = first;
            node._prevnode = null;
            node.SetParent(_parentnode);

            if (first != null) first._prevnode = node;
        }
