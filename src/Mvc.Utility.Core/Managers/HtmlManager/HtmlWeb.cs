...
/// </summary>
        ...
        public int StreamBufferSize
        {
            ...
            set
            {
                if (_streamBufferSize <= 0) throw new ArgumentException("Size must be greater than zero.");
        ...
        {
            get => CachePath != null && _usingCache;
            set
            {
                if (value && CachePath == null) throw new HtmlWebException("You need to define a CachePath first.");
        ...
        {
            if (string.IsNullOrEmpty(extension)) return def;

            if (!extension.StartsWith(".")) extension = "." + extension;

            if (!MimeTypeMap.Mappings.TryGetValue(extension, out var contentType)) contentType = def;
        ...
        {
            if (string.IsNullOrEmpty(contentType)) return def;

            if (contentType.StartsWith("."))
                throw new ArgumentException("Requested mime type is not valid: " + contentType);

            if (!MimeTypeMap.Mappings.TryGetValue(contentType, out var ext)) ext = def;
...
/// <param name="credentials"></param>
        ...
        {
            Get(url: url, path: path, proxy: proxy, credentials: credentials, method: "GET");
        }
#endif
...
// TODO: Check if UriSchemeHttps is still internal in NETSTANDARD 2.0
...
#endif
            {
                Get(uri: uri, method: method, path: path, doc: null, proxy: proxy, creds: credentials);
            }
            else
            {
                throw new HtmlWebException(message: "Unsupported uri scheme: '" + uri.Scheme + "'.");
            }
        }
#endif
        ...
        public string GetCachePath(Uri uri)
        {
            ...
            if (!UsingCache) throw new HtmlWebException("Cache is not enabled. Set UsingCache to true first.");
            ...
            else
            {
                ...
                foreach (var c in invalid)
                    absolutePathWithoutBadChar =
                        absolutePathWithoutBadChar.Replace(c.ToString(), "");
                ...
                if (uri.AbsolutePath[uri.AbsolutePath.Length - 1] == Path.AltDirectorySeparatorChar)
                    ...
                    cachePath = Path.Combine(CachePath, uri.Host + absolutePathWithoutBadChar.Replace('/', '\\'));
            }
        ...
        public HtmlDocument Load(string url, string proxyHost, int proxyPort, string userId, string password)
        {
            ...
            if (userId != null && password != null)
            {
                ...
                {
                    //Add the creds
                    {myProxy.Address, "Basic", myCreds}, {myProxy.Address, "Digest", myCreds}
                };
            }
        ...
        {
            if (UsingCache) _usingCacheAndLoad = true;
...
// TODO: Check if UriSchemeHttps is still internal in NETSTANDARD 2.0
            ...
            if (PreHandleDocument != null) PreHandleDocument(doc);
...
/// <returns>A new HTML document.</returns>
        ...
        {
            var uri = new Uri(uriString: url);
            return Load(uri: uri, method: method, proxy: proxy, credentials: credentials);
        }
#endif
...
#endif
            {
                doc = LoadUrl(uri: uri, method: method, proxy: proxy, creds: credentials);
            }
            else
            {
#if !(NETSTANDARD1_3 || NETSTANDARD1_6)
...
// TODO: Check if UriSchemeHttps is still internal in NETSTANDARD 2.0
...
#endif
                {
                    ...
                    doc.OptionAutoCloseOnEnd = true;
                    doc.DetectEncodingAndLoad(path: uri.OriginalString, detectEncoding: AutoDetectEncoding);
                }
                else
                {
                    throw new HtmlWebException(message: "Unsupported uri scheme: '" + uri.Scheme + "'.");
            ...
            {
                PreHandleDocument(document: doc);
            }
...
/// <param name="writer">The XmlTextWriter to which you want to save to.</param>
        ...
        {
            var doc = Load(url: htmlUrl);
            doc.Save(writer: writer);
        }
#endif
                ...
                using (var br = new BinaryReader(stream))
                {
                    using (var bw = new BinaryWriter(fs))
                    {
                        ...
                        do
                        {
                            ...
                            len += buffer.Length;
                            if (buffer.Length > 0) bw.Write(buffer);
            ...
            ICredentials creds)
        {
            ...
            if (CaptureRedirect) req.AllowAutoRedirect = false;
            ...
            if (_cacheOnly || UsingCacheIfExists)
            {
                if (File.Exists(cachePath))
                {
                    if (path != null)
                ...
                if (_cacheOnly) throw new HtmlWebException("File was not found at cache path: '" + cachePath + "'");
            }

            if (UseCookies) req.CookieContainer = new CookieContainer();
            ...
            if (PostResponse != null) PostResponse(req, resp);
            ...
            if (s != null)
            {
                if (UsingCache)
                {
                    ...
                    if (_usingCacheAndLoad) doc.Load(cachePath);
                }
                else
                {
        ...
        private HtmlDocument LoadUrl(Uri uri, string method, IWebProxy proxy, ICredentials creds)
        {
            var doc = new HtmlDocument
            {
                    OptionAutoCloseOnEnd = false, OptionFixNestedTags = true
            };
            StatusCode = Get(uri: uri, method: method, path: null, doc: doc, proxy: proxy, creds: creds);
            if (StatusCode == HttpStatusCode.NotModified)
            {
                // read cached encoding
                doc.DetectEncodingAndLoad(path: GetCachePath(uri: uri));
            }
        ...
        {
            // we cache the original headers aside the cached document.
            var file = GetCacheHeadersPath(uri: requestUri);
            var doc = new XmlDocument();
            doc.LoadXml(xml: "<c></c>");
            var cache = doc.FirstChild;
            foreach (var header in resp.Headers)
            {
                XmlNode entry = doc.CreateElement(name: "h");
                var att = doc.CreateAttribute(name: "n");
                att.Value = header.Key;
                entry.Attributes.Append(node: att);

                att = doc.CreateAttribute(name: "v");
                att.Value = string.Join(separator: ";", values: header.Value);
                entry.Attributes.Append(node: att);

                cache.AppendChild(newChild: entry);
            }

            doc.Save(outStream: File.OpenWrite(path: file));
        }
#endif
