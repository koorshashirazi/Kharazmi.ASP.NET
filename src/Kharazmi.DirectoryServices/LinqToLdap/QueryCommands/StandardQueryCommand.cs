...
namespace Kharazmi.DirectoryServices.Ldap.LinqToLdap.QueryCommands
{
    internal class StandardQueryCommand : QueryCommand
    {
            ...
            bool pagingEnabled, ILinqToLdapLogger log = null, string namingContext = null)
        {
            ...
            if (Options.WithoutPaging || !pagingEnabled ||
                (pageRequest = GetControl<PageResultRequestControl>(SearchRequest.Controls)) == null &&
                Options.PagingOptions == null)
        ...
        public virtual object HandleStandardRequest(DirectoryConnection connection, ILinqToLdapLogger log, int maxSize,
            bool pagingEnabled)
        {
            ...
            if (pagingEnabled)
            {
                var maxPageSize = Options.PageSize ?? maxSize;
            ...
            if (pagingEnabled)
                if (response.Entries.Count > 0)
                {
                    ...
                    var hasMoreResults = pageResultResponseControl != null &&
                                         pageResultResponseControl.Cookie.Length > 0 &&
                                         (!Options.TakeSize.HasValue || list.Count < Options.TakeSize.Value);
                    while (hasMoreResults)
                    {
                        ...
                        hasMoreResults = pageResultResponseControl != null &&
                                         pageResultResponseControl.Cookie.Length > 0 &&
                                         (!Options.TakeSize.HasValue || list.Count <= Options.TakeSize.Value);
        ...
        public virtual object HandlePagedRequest(DirectoryConnection connection, PageResultRequestControl pageRequest,
            ILinqToLdapLogger log)
        {
            if (Options.YieldNoResults)
            {
                var bindingAttr = new[]
                ...
                return Activator.CreateInstance(typeof(LdapPage<>).MakeGenericType(Options.GetEnumeratorReturnType()),
                    bindingAttr);
            }
        ...
        public virtual object HandleVlvRequest(DirectoryConnection connection, int maxSize, ILinqToLdapLogger log)
        {
            if (Options.YieldNoResults)
            {
                ...
                return Activator.CreateInstance(
                    typeof(VirtualListView<>).MakeGenericType(Options.GetEnumeratorReturnType()), bindingAttr);
            }

            if (GetControl<SortRequestControl>(SearchRequest.Controls) == null)
                throw new InvalidOperationException(
                    "Virtual List Views require a sort operation. Please include an OrderBy in your query.");
            ...
            if (vlvResponse == null)
                throw new InvalidOperationException(
                    "The server does not support Virtual List Views. Skip cannot be used. Please use standard paging.");
            var parameters = new[]
            {
