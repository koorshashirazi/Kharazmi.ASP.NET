...
namespace Kharazmi.DirectoryServices.Ldap.LinqToLdap.Mapping.PropertyMappings
{
    internal class CustomPropertyMapping<T, TProperty> : PropertyMappingGeneric<T> where T : class
    {
        ...
        public CustomPropertyMapping(PropertyMappingArguments<T> arguments,
            Func<DirectoryAttribute, TProperty> fromConverter, Func<TProperty, object> toConverter,
            Func<TProperty, string> toFilterConverter, Func<TProperty, TProperty, bool> isEqual)
            : base(arguments)
        {
        ...
        public override object FormatValueFromDirectory(DirectoryAttribute value, string dn)
        {
            if (_convertFrom == null)
                throw new MappingException(
                    string.Format("No from directory converter defined for mapped property '{0}' on '{1}'",
                        PropertyName,
                        typeof(T).FullName));
        ...
        public override DirectoryAttributeModification GetDirectoryAttributeModification(object instance)
        {
            ...
            if (value != null)
            {
                if (value is byte[])
                    ...
                    throw new MappingException(string.Format("Unsupported value '{0}' for property '{1}' on '{2}'",
                        value, PropertyName, typeof(T).FullName));
            }
