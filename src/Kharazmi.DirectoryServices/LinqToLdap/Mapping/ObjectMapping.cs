...
namespace Kharazmi.DirectoryServices.Ldap.LinqToLdap.Mapping
{
    internal abstract class ObjectMapping : IObjectMapping
    {
        ...
        protected ObjectMapping(string namingContext, IEnumerable<IPropertyMapping> propertyMappings,
            string objectCategory = null, bool includeObjectCategory = true, IEnumerable<string> objectClass = null,
            bool includeObjectClasses = true)
        {
            ...
            try
            {
                _attributePropertyMappings =
                    localPropertyMappings
                        .ToDictionary(pm => pm.AttributeName, pm => pm, StringComparer.OrdinalIgnoreCase)
            ...
            _catchAll =
                localPropertyMappings.FirstOrDefault(p =>
                    typeof(IDirectoryAttributes).IsAssignableFrom(p.PropertyType));
...
#if NET45
        public System.Collections.ObjectModel.ReadOnlyDictionary<string, string> Properties => _propertyNames ?? (_propertyNames
 = InitializePropertyNames());
#else
        public Collections.ReadOnlyDictionary<string, string> Properties =>
            _propertyNames ?? (_propertyNames = InitializePropertyNames());
#endif

        public ReadOnlyCollection<IObjectMapping> SubTypeMappings => _readOnlySubTypeMappings ??
                                                                     (_readOnlySubTypeMappings =
                                                                         new ReadOnlyCollection<IObjectMapping>(
                                                                             SubTypeMappingsObjectClassDictionary.Values
                                                                                 .ToList()));
        ...
        public IPropertyMapping GetPropertyMapping(string name, Type owningType = null)
        {
            if (owningType == null || owningType == Type)
            {
                ...
                if (_propertyMappings.TryGetValue(name, out mapping)) return mapping;
                if (HasSubTypeMappings) return null;
            }
        ...
        public IPropertyMapping GetPropertyMappingByAttribute(string name, Type owningType = null)
        {
            ...
            {
                IPropertyMapping mapping;
                if (_attributePropertyMappings.TryGetValue(name, out mapping)) return mapping;
            }
            else if (HasSubTypeMappings)
            {
                IObjectMapping subTypeMapping;
        ...
        public virtual void AddSubTypeMapping(IObjectMapping mapping)
        {
            ...
            var currentMappings =
                SortByInheritanceDescending(SubTypeMappingsObjectClassDictionary.Values.Union(new[] {mapping}));
            ...
            foreach (var currentMapping in currentMappings)
            {
                ...
                if (objectClasses.Count == 0)
                    throw new InvalidOperationException(
                        "Unable to identify distinct object class based on mapped inheritance");
...
#endif
        {
            var properties = _propertyMappings.ToDictionary(x => x.Key, x => x.Value.AttributeName,
        ...
        {
            var hiearchy = new Dictionary<int, List<IObjectMapping>>();
