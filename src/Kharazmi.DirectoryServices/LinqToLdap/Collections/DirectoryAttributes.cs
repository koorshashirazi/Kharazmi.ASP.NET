...
namespace Kharazmi.DirectoryServices.Ldap.LinqToLdap.Collections
{
    ...
    public sealed class DirectoryAttributes : IDirectoryAttributes
    {
        ...
        ///     The attributes for the entry that will be treated as changed attributes.  Attributes named distinguishedname and
        ///     entrydn are ignored.
        /// </param>
        public DirectoryAttributes(string distinguishedName = null,
            IEnumerable<KeyValuePair<string, object>> attributes = null)
        {
            ...
            foreach (var keyValuePair in attributes)
            {
                ...
                if (name.Equals("distinguishedname", StringComparison.OrdinalIgnoreCase) ||
                    name.Equals("entrydn", StringComparison.OrdinalIgnoreCase)) continue;
        ...
        public ReadOnlyCollection<string> AttributeNames => _attributeNames ??
                                                            (_entry != null
                                                                ...
                                                                ? _attributeNames =
                                                                    new ReadOnlyCollection<string>(_entry.Attributes
                                                                        .AttributeNames.Cast<string>().ToList())
        ...
        ///     Thrown if a modification with the same name and operation has already been
        ///     added.
        /// </exception>
        public IDirectoryAttributes AddModification(DirectoryAttributeModification modification)
        {
            ...
            if (name.Equals("distinguishedname", StringComparison.OrdinalIgnoreCase) ||
                name.Equals("entrydn", StringComparison.OrdinalIgnoreCase))
                throw new ArgumentException(
                    "Cannot change the distinguished name. Please use MoveEntry or RenameEntry.");

            if (_changedAttributes == null) _changedAttributes = new List<DirectoryAttributeModification>();

            var changed = _changedAttributes
                .FirstOrDefault(c =>
                    c.Name.Equals(name, StringComparison.OrdinalIgnoreCase) && c.Operation == modification.Operation);
        ...
        public string DistinguishedName => _entry != null
            ...
            : _distinguishedName.IsNullOrEmpty()
                ? string.Empty
                : _distinguishedName;
        ...
        /// </summary>
        /// <exception cref="InvalidOperationException">
        ///     Thrown if an attempt is made to access the Entry when this instance was not
        ///     create from a <see cref="SearchResultEntry" />.
        /// </exception>
        ...
        public object GetValue(string attribute)
        {
            ...
            if (directoryAttribute != null)
            {
                if (directoryAttribute.Count > 1)
                {
                    ...
                    if (type == typeof(string)) return GetStrings(attribute).ToList();
                    if (type == typeof(byte[])) return GetByteArrays(attribute).ToList();
                }

                if (attribute.EndsWith("guid", StringComparison.OrdinalIgnoreCase) ||
                ...
                if ("photo".Equals(attribute, StringComparison.OrdinalIgnoreCase)) return GetBytes(attribute);
        ...
        public IEnumerable<string> GetStrings(string attribute)
        {
            ...
            {
                var valueArray = directoryAttribute.GetValues(type);
                var rawValue = valueArray.Length == 0
                    ? null
                    : valueArray;
                if (rawValue != null)
        ...
        /// <param name="attribute">The name of the attribute</param>
        /// <returns></returns>
        /// <exception cref="Mvc.Utility.Core.Managers.LinqToLdap.Exceptions.MappingException">
            ...
            if (directoryAttribute != null)
            {
                ...
                try
                {
                    ...
                    value = rawValue == null || Equals("9223372036854775807", rawValue)
                        ? default(DateTime?)
                        : format == null
                            ? DateTime.FromFileTime(long.Parse(rawValue.ToString()))
                            : rawValue.FormatLdapDateTime(format);
                }
                catch (Exception ex)
                {
                    ThrowFormatException(ex, attribute, rawValue, type);
        ...
        public IEnumerable<X509Certificate2> GetX509Certificate2s(string attribute)
        {
            ...
            if (directoryAttribute != null)
            {
                ...
                try
                {
                    ...
                    for (var i = 0; i < valueArray.Length; i++) value[i] = new X509Certificate2((byte[]) valueArray[i]);
                }
                catch (Exception ex)
                {
#if NET35
        ...
        public Guid? GetGuid(string attribute)
        {
            ...
            try
            {
                ...
                if (value != null) return new Guid(value);
            }
            catch (Exception ex)
            {
#if NET35
        ...
        public SecurityIdentifier GetSecurityIdentifier(string attribute)
        {
            ...
            {
                value = GetBytes(attribute);
                if (value != null) return new SecurityIdentifier(value, 0);
            }
            catch (Exception ex)
            {
#if NET35
...
#else
                ThrowFormatException(ex, attribute, value == null ? "" : string.Join(",", value),
...
#else
                ThrowFormatException(ex, attribute, value == null ? "" : string.Join(",", value),
                    typeof(IEnumerable<SecurityIdentifier>));
#endif
            }
        ...
        public IDirectoryAttributes SetNull(string attribute)
        {
            return AddModification(
                ExtensionMethods.ToDirectoryModification(null, attribute, DirectoryAttributeOperation.Replace));
        }
