...
namespace Kharazmi.DirectoryServices.Ldap.LinqToLdap.Logging
{
    internal static class LoggingExtensions
    {
        ...
        internal static string ToLogString(this DirectoryResponse response)
        {
            ...
            foreach (var control in response.Controls) control.AppendTo(sb);
            sb.AppendLine();
            return sb.ToString();
        }
        ...
        internal static string ToLogString(this SearchRequest searchRequest)
        {
            ...
            sb.AppendLine("Controls: ");
            foreach (DirectoryControl control in searchRequest.Controls) control.AppendTo(sb);
            sb.AppendLine();
            return sb.ToString();
        }
        ...
        internal static string ToLogString(this AddRequest addRequest)
        {
            ...
            sb.AppendLine("Attributes: " + string.Join(", ",
                              addRequest.Attributes.Cast<DirectoryAttribute>().Select(FormatDirectoryAttribute)));
#endif
            sb.AppendLine("Request ID: " + addRequest.RequestId);
            sb.AppendLine("Controls: ");
            foreach (DirectoryControl control in addRequest.Controls) control.AppendTo(sb);
        ...
        internal static string ToLogString(this DeleteRequest deleteRequest)
        {
            ...
            sb.AppendLine("Controls: ");
            foreach (DirectoryControl control in deleteRequest.Controls) control.AppendTo(sb);
...
#else
            sb.AppendLine("Attributes: " + string.Join(", ",
                              modifyRequest.Modifications.Cast<DirectoryAttributeModification>()
                                  .Select(FormatDirectoryAttribute)));
#endif
            sb.AppendLine("Request ID: " + modifyRequest.RequestId);
            sb.AppendLine("Controls: ");
            foreach (DirectoryControl control in modifyRequest.Controls) control.AppendTo(sb);
        ...
        internal static string ToLogString(this ModifyDNRequest modifyDNRequest)
        {
            ...
            sb.AppendLine("Controls: ");
            foreach (DirectoryControl control in modifyDNRequest.Controls) control.AppendTo(sb);
        ...
        internal static void AppendTo(this DirectoryControl control, StringBuilder sb)
        {
            ...
            if (control is VlvRequestControl)
            {
                ...
                sb.AppendLine("Target: " + (vlvControl.Target == null || vlvControl.Target.Length == 0
                                  ? ""
                                  : Encoding.UTF8.GetString(vlvControl.Target)));
        ...
        private static string FormatDirectoryAttribute(DirectoryAttributeModification directoryAttribute)
        {
            if (directoryAttribute != null)
            {
                var name = directoryAttribute.Name;
                string value = null;
