...
namespace Kharazmi.DirectoryServices.Ldap.LinqToLdap.Visitors
{
        ...
        protected override Expression VisitBinary(BinaryExpression b)
        {
            ...
            if (b.NodeType == ExpressionType.AndAlso)
            {
                    ...
                    b.Right.Type == typeof(bool))
                {
                    ...
                    if (leftValue == rightValue) return Visit(b.Left);
                    if (false.Equals(leftValue)) return Visit(b.Left);
                    if (false.Equals(rightValue)) return Visit(b.Left);
                }

                if (b.Left.NodeType == ExpressionType.Constant && b.Left.Type == typeof(bool))
                {
                    var value = (b.Left as ConstantExpression).Value;
                    if (false.Equals(value)) return Visit(b.Left);
                    if (true.Equals(value)) return Visit(b.Right);
                }
                else if (b.Right.NodeType == ExpressionType.Constant && b.Right.Type == typeof(bool))
                {
                    var value = (b.Right as ConstantExpression).Value;
                    if (false.Equals(value)) return Visit(b.Right);
                    if (true.Equals(value)) return Visit(b.Left);
                }
            }
