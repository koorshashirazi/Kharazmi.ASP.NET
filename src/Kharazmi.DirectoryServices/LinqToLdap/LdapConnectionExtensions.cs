...
namespace Kharazmi.DirectoryServices.Ldap.LinqToLdap
{
    ...
    public static class LdapConnectionExtensions
    {
        ...
        ///     Thrown if <paramref name="connection" /> is null or
        ///     <paramref name="namingContext" /> is null, empty, or white space.
        /// </exception>
        /// <returns></returns>
        /// <example>
        ...
        public static IQueryable<IDirectoryAttributes> Query(this LdapConnection connection, string namingContext,
            SearchScope scope = SearchScope.Subtree, ILinqToLdapLogger log = null, int maxPageSize = 500,
            bool withPaging = true)
        {
            if (connection == null) throw new ArgumentNullException("connection");
            var provider = new DirectoryQueryProvider(connection, scope,
                new DynamicObjectMapping(namingContext, null, null), withPaging)
            {
                Log = log,
        ...
        /// <returns></returns>
        /// <exception cref="ArgumentNullException">
        ///     Thrown if <paramref name="connection" /> or <paramref name="entry" />> is null.
        /// </exception>
            ...
            DirectoryControl[] controls = null, IEnumerable<IAddEventListener> listeners = null)
        {
            ...
            try
            {
                ...
                var request = new AddRequest(distinguishedName,
                    entry.GetChangedAttributes().Where(da => da.Count > 0).ToArray());
                ...
                if (listeners != null)
                {
                    ...
                    foreach (var eventListener in listeners.OfType<IPreAddEventListener>()) eventListener.Notify(args);
                }
                ...
                if (listeners != null)
                {
                    var args = new ListenerPostArgs<object, AddRequest, AddResponse>(entry, request, response,
                        connection);
                    foreach (var eventListener in listeners.OfType<IPostAddEventListener>()) eventListener.Notify(args);
                }
            }
            catch (Exception ex)
            {
        ...
        public static IDirectoryAttributes AddAndGet(this LdapConnection connection, IDirectoryAttributes entry,
            ILinqToLdapLogger log = null, DirectoryControl[] controls = null,
            IEnumerable<IAddEventListener> listeners = null)
        {
        ...
        /// <exception cref="LdapException">Thrown if the operation fails.</exception>
        public static void Delete(this LdapConnection connection, string distinguishedName,
            ILinqToLdapLogger log = null, DirectoryControl[] controls = null,
            IEnumerable<IDeleteEventListener> listeners = null)
        {
            try
            {
                if (connection == null) throw new ArgumentNullException("connection");
                ...
                if (listeners != null)
                {
                    var args = new ListenerPostArgs<string, DeleteRequest, DeleteResponse>(distinguishedName, request,
                        response, connection);
            ...
            {
                var message = string.Format("An error occurred while trying to delete '{0}'.", distinguishedName);
        ...
        public static void Update(this LdapConnection connection, IDirectoryAttributes entry,
            ILinqToLdapLogger log = null, DirectoryControl[] controls = null,
            IEnumerable<IUpdateEventListener> listeners = null)
        {
            string distinguishedName = null;
            try
            {
                ...
                if (changes.Any())
                {
                    ...
                    if (listeners != null)
                    {
                        var args = new ListenerPostArgs<object, ModifyRequest, ModifyResponse>(entry, request, response,
                            connection);
        ...
        /// <exception cref="DirectoryOperationException">Thrown if the operation fails</exception>
        /// <exception cref="LdapException">Thrown if the operation fails</exception>
        public static IDirectoryAttributes UpdateAndGet(this LdapConnection connection, IDirectoryAttributes entry,
            ILinqToLdapLogger log = null, DirectoryControl[] controls = null,
            IEnumerable<IUpdateEventListener> listeners = null)
        {
        ...
        /// </param>
        /// <returns></returns>
        public static IDirectoryAttributes ListServerAttributes(this LdapConnection connection,
            string[] attributes = null, ILinqToLdapLogger log = null)
        {
            try
            {
                if (connection == null) throw new ArgumentNullException("connection");
                    ...
                    connection, SearchScope.Base, new ServerObjectMapping(), false) {Log = log, IsDynamic = true})
                {
                    ...
                    if (attributes != null && attributes.Length > 0) query = query.Select(attributes);
        ...
        /// <returns></returns>
        public static IDirectoryAttributes GetByDN(this LdapConnection connection, string distinguishedName,
            ILinqToLdapLogger log = null, params string[] attributes)
        {
            try
            {
                if (connection == null) throw new ArgumentNullException("connection");
        ...
        ///     Defaults to null to use default behavior
        /// </param>
        /// <param name="controls">Any <see cref="DirectoryControl" />s to be sent with the request</param>
        ...
        public static string MoveEntry(this LdapConnection connection, string currentDistinguishedName,
            string newNamingContext, ILinqToLdapLogger log = null, bool? deleteOldRDN = null,
            params DirectoryControl[] controls)
        {
            try
            {
                ...
                var response = SendModifyDnRequest(connection, currentDistinguishedName, newNamingContext, name,
                    deleteOldRDN, controls, log);
                response.AssertSuccess();
            ...
            catch (Exception ex)
            {
                if (log != null)
                    log.Error(ex,
                        string.Format("An error occurred while trying to move entry '{0}' to '{1}'.",
                            currentDistinguishedName, newNamingContext));
        ...
        /// <exception cref="DirectoryOperationException">Thrown if the operation fails.</exception>
        /// <exception cref="LdapConnection">Thrown if the operation fails.</exception>
        public static string RenameEntry(this LdapConnection connection, string currentDistinguishedName,
            string newName, ILinqToLdapLogger log = null, bool? deleteOldRDN = null, params DirectoryControl[] controls)
        {
            try
            {
                if (connection == null) throw new ArgumentNullException("connection");
                ...
                var response = SendModifyDnRequest(connection, currentDistinguishedName, container, newName,
                    deleteOldRDN, controls, log);
                response.AssertSuccess();
            ...
            catch (Exception ex)
            {
                if (log != null)
                    log.Error(ex,
                        string.Format("An error occurred while trying to rename entry '{0}' to '{1}'.",
                            currentDistinguishedName, newName));

                throw;
            }
        }

        private static ModifyDNResponse SendModifyDnRequest(LdapConnection connection, string dn, string parentDn,
            string newName, bool? deleteOldRDN, DirectoryControl[] controls, ILinqToLdapLogger log = null)
        {
            ...
            if (deleteOldRDN.HasValue) request.DeleteOldRdn = deleteOldRDN.Value;
            if (controls != null) request.Controls.AddRange(controls);
            if (log != null && log.TraceEnabled) log.Trace(request.ToLogString());
        ...
        /// </exception>
        /// <returns></returns>
        public static IList<TValue> RetrieveRanges<TValue>(this LdapConnection connection, string distinguishedName,
            string attributeName, int start = 0, ILinqToLdapLogger log = null)
        {
            var idx = start;
            var step = 0;
...
// ReSharper restore PossibleNullReferenceException
            ...
            catch (Exception ex)
            {
                if (log != null)
                    log.Error(ex,
                        string.Format(
                            "An error occurred while trying to retrieve ranges of type '{0}' for range '{1}'.",
                            typeof(TValue).FullName, currentRange));
