...
namespace Kharazmi.DirectoryServices.Ldap.LinqToLdap.Helpers
{
    ...
    internal static class DelegateBuilder
    {
        ...
        private static Type GetConcreteType(Type type)
        {
            if (type.IsInterface)
                if (type.IsGenericType)
                {
                    ...
                    if (listType != null) type = listType.MakeGenericType(type.GetGenericArguments());
                }
        ...
        private static Ctor<T> BuildCtor<T>(Type concreteType)
        {
            ...
            if (ctor == null)
            {
                var message = "No parameterless constructor found for " + typeof(T).FullName;
                return delegate { throw new MissingConstructorException(message); };
            }
        ...
        {
            var type = GetConcreteType(typeof(T));
