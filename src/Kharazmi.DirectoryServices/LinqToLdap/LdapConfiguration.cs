...
namespace Kharazmi.DirectoryServices.Ldap.LinqToLdap
{
    ...
    public class LdapConfiguration : ILdapConfiguration
    {
...
#else
        private readonly ConcurrentBag<IEventListener> _listeners = new ConcurrentBag<IEventListener>();
#endif
        ...
        public void RegisterListeners(params IEventListener[] listeners)
        {
            ...
            foreach (var listener in listeners) _listeners.Add(listener);
        }
        ...
        /// </summary>
        public int ServerMaxPageSize { get; private set; } = 500;

        /// <summary>
        ///     Indicates if paging is enabled.
        /// </summary>
        public bool PagingEnabled { get; private set; } = true;
        ...
        /// <returns></returns>
        public LdapConfiguration AddMapping(IClassMap classMap, string namingContext = null,
            IEnumerable<string> objectClasses = null, bool includeObjectClasses = true, string objectCategory = null,
            bool includeObjectCategory = true)
        {
            Mapper.Map(classMap, namingContext, objectClasses, includeObjectClasses, objectCategory,
        ...
        {
            if (Configuration != null) throw new MappingException("Static storage has already been set.");
            Configuration = this;
        ...
        /// <summary>
        ///     Sets the logger for <see cref="ConnectionFactory" /> and <see cref="DirectoryContext" />s created or configured by
        ///     this class.
        /// </summary>
        ...
        public LdapConfiguration LogTo(ILinqToLdapLogger log)
        {
            ...
            if (ConnectionFactory is ConnectionFactoryBase) (ConnectionFactory as ConnectionFactoryBase).Logger = Log;
