...
namespace Kharazmi.AspNetMvc.Core.Managers.StateManager
{
    ...
    public partial class StateMachine<TState, TTrigger>
    {
        ...
        private void InternalFireOne(TTrigger trigger, params object[] args)
        {
            ...
            if (_triggerConfiguration.TryGetValue(trigger, out configuration)) configuration.ValidateParameters(args);
        ...
        {
            return string.Format("StateMachine {{ State = {0}, PermittedTriggers = {{ {1} }}}}", State,
        ...
        private void SaveTriggerConfiguration(TriggerWithParameters trigger)
        {
            if (_triggerConfiguration.ContainsKey(trigger.Trigger))
                throw new InvalidOperationException(string.Format(StateMachineResources.CannotReconfigureParameters,
                    trigger));
        ...
        private void DefaultUnhandledTriggerAction(TState state, TTrigger trigger)
        {
            ...
            if (representativeState.TryFindHandlerWithUnmetGuardCondition(trigger, out triggerBehaviour))
                throw new InvalidOperationException(string.Format(
                    StateMachineResources.NoTransitionsUnmetGuardCondition, trigger, state,
                    triggerBehaviour.GuardDescription));

            throw new InvalidOperationException(string.Format(StateMachineResources.NoTransitionsPermitted, trigger,
                state));
        }
