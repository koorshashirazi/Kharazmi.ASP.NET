...
using Kharazmi.AspNetMvc.Core.Managers.Authorization.Infrastructure;
...
namespace Kharazmi.AspNetMvc.Core.Managers.Authorization
{
    ...
    public class DefaultAuthorizationService : IAuthorizationService
    {
        ...
        public DefaultAuthorizationService(IAuthorizationPolicyProvider policyProvider,
            ...
            : this(policyProvider, handlers, null, new DefaultAuthorizationHandlerContextFactory(),
        ...
        public DefaultAuthorizationService(IAuthorizationPolicyProvider policyProvider,
            IEnumerable<IAuthorizationHandler> handlers, ILogger logger,
        ...
        ///     .
        /// </returns>
        public async Task<bool> AuthorizeAsync(ClaimsPrincipal user, object resource,
            IEnumerable<IAuthorizationRequirement> requirements)
        {
            if (requirements == null) throw new ArgumentNullException(nameof(requirements));

            var authContext = _contextFactory.CreateContext(requirements, user, resource);
            foreach (var handler in _handlers) await handler.HandleAsync(authContext);
        ...
        public async Task<bool> AuthorizeAsync(ClaimsPrincipal user, object resource, string policyName)
        {
            ...
            if (policy == null)
                throw new InvalidOperationException(
                    ResourceHelper.FormatException_AuthorizationPolicyNotFound(policyName));
        ...
        private static IList<IAuthorizationHandler> InitializeHandlers(IEnumerable<IAuthorizationHandler> handlers)
        {
            ...
            {
                if (handler is PassThroughAuthorizationHandler) passThroughFound = true;

                allHandlers.Add(handler);
            }

            if (!passThroughFound) allHandlers.Add(new PassThroughAuthorizationHandler());
        ...
        private static string GetUserNameForLogging(ClaimsPrincipal user)
        {
            ...
            if (identity != null)
            {
                ...
                return GetClaimValue(identity, "sub") ?? GetClaimValue(identity, ClaimTypes.Name) ??
