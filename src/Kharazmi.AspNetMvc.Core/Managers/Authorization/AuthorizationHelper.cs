...
namespace Kharazmi.AspNetMvc.Core.Managers.Authorization
{
    ...
    public class AuthorizationHelper : IResourceAuthorizationHelper
    {
        ...
        {
            _owinContextAccessor = owinContextAccessor ?? throw new ArgumentNullException(nameof(owinContextAccessor));
        }
        ...
        /// </remarks>
        public async Task<bool> IsAuthorizedAsync(IAuthorizationController controller, ClaimsPrincipal user,
            IAuthorizeData authorizeAttribute)
        {
            if (user == null) throw new ArgumentNullException(nameof(user));

            if (authorizeAttribute == null) throw new ArgumentNullException(nameof(authorizeAttribute));

            var options = ResolveAuthorizationOptions(controller);

            var dependencies = options.Dependencies ?? new AuthorizationDependencies();
            var policyProvider = dependencies.PolicyProvider ?? new DefaultAuthorizationPolicyProvider(options);
        ...
        {
            if (controller != null) return controller.AuthorizationOptions;
