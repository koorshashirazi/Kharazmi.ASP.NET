...
using System.Text;
using Kharazmi.AspNetMvc.Core.Managers.PdfManager.Assets;
using Kharazmi.HtmlToPdf;
...
namespace Kharazmi.AspNetMvc.Core.Managers.PdfManager
{
    internal static class HtmlToPdfConverterProcess
    {
        ...
        {
            if (process.ExitCode != 0) throw new PdfDocumentCreationFailedException(process.StandardError.ReadToEnd());
        }
        ...
        private static ProcessStartInfo GetProcessStartInfo()
        {
            ...
            {
                FileName = ConverterExecutable.Get().FullConverterExecutableFilename, RedirectStandardInput = true,
            ...
            , IDictionary<string, string> objectSettings)
        {
            ...
            {
                Html = html, GlobalSettings = globalSettings, ObjectSettings = objectSettings
            };
