﻿using Mvc.Utility.Core.Managers.Authorization;
...
namespace Mvc.Utility.Core.Test
{
    ...
    public static class AssignableToRolePermissions
    {
        ...
        private static IEnumerable<PermissionModel> GetPermision() => new List<PermissionModel>
            {
        ...
        private static IEnumerable<string> GetPermisionNames() => new List<string>()
            {
        ...
        public static IEnumerable<SelectListItem> GetAsSelectListItems() => Permissions.Select(a => new SelectListItem { Text = a.Description, Value = a.Name }).ToList();
        #endregion

        public static IEnumerable<string> GetBaseSettingPermissions() => new List<string>
            {
                CanManageTrainingCenter,
                CanManageTrainingCourse,
                CanManageTitle,
    ...
    public sealed class PermissionResource : Singleton<PermissionResource>
    {
    ...
    {
        bool HasPermission(ClaimsPrincipal user, List<PermissionModel> permission);
    }

    public class PermissionProvider : IPermissionProvider
    {
        public bool HasPermission(ClaimsPrincipal user, List<PermissionModel> permissions)
        {
            if (user == null)
            {
            ...
            var userPermissions = user.FindFirst("permissions").Value.Split(',').ToList();
            // انجام مقایسات
    ...
    public class PermissionRequirement : AuthorizationHandler<PermissionRequirement>, IAuthorizationRequirement
    {
        ...
        private readonly IPermissionProvider _permissionResource;
        private readonly List<PermissionModel> _permission;

        public PermissionRequirement(IPermissionProvider permissionResource, List<PermissionModel> permission)
        {
            _permissionResource = permissionResource ?? throw new ArgumentNullException(nameof(permissionResource));
        ...
        protected override async Task HandleRequirementAsync(AuthorizationHandlerContext context, PermissionRequirement requirement)
        {
            ...
            var hasPermission = _permissionResource.HasPermission(context.User, requirement._permission);
            if (hasPermission)
            {
